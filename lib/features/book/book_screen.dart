    ...
    return properties.where((property) {
      ...
      final haystack =
          '${property.name} ${property.location} ${property.type} ${property.tags.join(' ')}'
              .toLowerCase();
      return matchesType && matchesBudget && haystack.contains(query);
    }).toList();
  }
      ...
      return Scaffold(
        ...
        body: const Center(
          child: CircularProgressIndicator(
            valueColor: AlwaysStoppedAnimation<Color>(Color(0xFFD4A847)),
          ),
        ),
      );
    }
...
class _RenterPropertyCard extends StatelessWidget {
          ...
          builder: (dialogContext) {
            return AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  Text(
                    'Request booking for ${property.name}?',
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      hintText: 'Optional message for owner',
    ...
    try {
      ...
      if (existing is List && existing.isNotEmpty) {
        if (context.mounted) {
          ...
          final info = status == 'accepted'
              ? 'Your booking is already accepted for this property.'
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context)
          ...
          ..showSnackBar(
            const SnackBar(
              content: Text('Request sent successfully. Owner will review and respond soon.'),
            ),
          );
      }
  ...
  Future<void> _showOwnerConnectSheet(BuildContext context) async {
    final hasPhone = (property.ownerPhone ?? '').isNotEmpty;
    final hasEmail = (property.ownerEmail ?? '').isNotEmpty;
            ...
            children: [
              ...
              if (hasPhone)
                ListTile(
                  ...
                  trailing: TextButton(
                    onPressed: () async {
                      await Clipboard.setData(ClipboardData(text: property.ownerPhone!));
                      if (context.mounted) {
                        Navigator.of(context).pop();
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Owner phone copied')),
                        );
                      }
                    },
              ...
              if (hasEmail)
                ListTile(
                  ...
                  trailing: TextButton(
                    onPressed: () async {
                      await Clipboard.setData(ClipboardData(text: property.ownerEmail!));
                      if (context.mounted) {
                        Navigator.of(context).pop();
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Owner email copied')),
                        );
                      }
                    },
                    child: const Text('Copy'),
              ...
              if (!hasPhone && !hasEmail)
                const Padding(
                  ...
                  child: Text(
                    'Owner contact details are not available yet.',
                    style: TextStyle(color: Colors.black54, fontSize: 13),
                  ),
                ),
            ],
                  ...
                  child: Row(
                    children: [
                      ...
                      Wrap(
                        ...
                        children: [
                          TextButton.icon(
                            onPressed: () => _showOwnerConnectSheet(context),
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 10),
                    Text('User Type: $renterType',
                        style: const TextStyle(fontSize: 12, color: Colors.black87)),
                    const SizedBox(height: 6),
                    Text('Preferred City: $city',
                        style: const TextStyle(fontSize: 12, color: Colors.black87)),
                    const SizedBox(height: 6),
                    Text('Budget Preference: $budget',
                        style: const TextStyle(fontSize: 12, color: Colors.black87)),
                    const SizedBox(height: 10),
                    const Text(
